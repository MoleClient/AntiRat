...
/** Minecraft 1.21.1-1.21.4 client adapter. */
public final class AntiRatClient implements ClientModInitializer {
    ...
    public static void postSystemMessage(String value, UiColor color) {
        ...
        client.execute(() -> {
            if (client.inGameHud != null) client.inGameHud.getChatHud().addMessage(Text.literal("[AntiRat] ")
                    .formatted(Formatting.DARK_GRAY).append(Text.literal(value).formatted(formatting(color))));
        });
    }
    ...
    private static void notifyClient(MinecraftClient client, ThreatEvent event) {
        ...
        if (event.blocked()) {
            ...
            client.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.BLOCK_NOTE_BLOCK_BIT, pitch));
            client.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.BLOCK_NOTE_BLOCK_PLING, 1.45F));
        } else if (event.riskLevel().atLeast(RiskLevel.HIGH)) {
            client.getSoundManager().play(PositionedSoundInstance.master(SoundEvents.BLOCK_NOTE_BLOCK_BELL, 1.1F));
        }
    }

    static MutableText chatMessage(ThreatEvent event) {
        Formatting risk = switch (event.riskLevel()) {
        ...
        MutableText details = Text.literal(event.type() == ThreatType.MOD_QUARANTINED
                ...
                .styled(style -> style.withColor(Formatting.AQUA).withUnderline(true)
                        .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
                                "/antirat show " + event.id()))
                        .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                Text.literal("Open this AntiRat event"))));
        return Text.literal("[AntiRat] ").formatted(Formatting.DARK_GRAY)
                .append(Text.literal((event.blocked() ? "prevented " : "flagged ") + event.type().label() + " from ")
                ...
                .append(Text.literal(event.riskLevel().label()).formatted(risk)).append(details);
    }
