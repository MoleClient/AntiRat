...
/** Minecraft 26.1-26.1.2 unobfuscated client adapter. */
public final class AntiRatClient implements ClientModInitializer {
    ...
    public static void onEndClientTick(Minecraft client) {
        ...
        if (startupPopup != null && client.screen == null) {
    ...
    public static void postSystemMessage(String value, UiColor color) {
        ...
        client.execute(() -> client.gui.getChat().addClientSystemMessage(Component.literal("[AntiRat] ")
    ...
    private static void notifyClient(Minecraft client, ThreatEvent event) {
        client.gui.getChat().addClientSystemMessage(chatMessage(event));
        if (event.blocked()) {
            client.gui.setOverlayMessage(Component.literal("AntiRat blocked: " + event.title())
